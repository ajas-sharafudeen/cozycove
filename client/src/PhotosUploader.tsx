...
import { useState, ChangeEvent, FormEvent, MouseEvent, SetStateAction, Dispatch } from "react";
...
export default function PhotosUploader({ addedPhotos, onChange }: PhotosUploaderProps) {
  ...
  async function addPhotoByLink(ev: FormEvent<HTMLButtonElement>) {
    ev.preventDefault();
    try {
  ...
  return (
    <>
      <div className="flex gap-2">
        ...
        />
        <button onClick={addPhotoByLink} className="bg-red-600 px-4 text-white font-bold rounded-2xl">
          Add&nbsp;Photo
        </button>
      </div>
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-3 lg:grid-cols-6">
        {addedPhotos.length > 0 && addedPhotos.map(link => (
          <div className="h-32 flex relative" key={link}>
            <img className="rounded-2xl w-full object-cover" src={`http://127.0.0.1:4000/uploads/${link}`} alt="" />
        ...
        <label className="h-32 cursor-pointer flex items-center gap-1 justify-center border bg-white rounded-2xl p-2 text-lg text-gray-600">
